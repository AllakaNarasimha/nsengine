...
class TvChart:
    ...
    def write_placeholder(self):
        if self.cfg.chart_library != 'tv':
            return None
        html_path = self.cfg.export_csv 
    ...
    def maybe_export(self, df, completed_trades, force=False):
        if self.cfg.chart_library != 'tv' or df is None or df.empty:
            return None
    ...
    def _export(self, df, completed_trades, final=False):
        try:
            ...
            html_template = self._apply_template_replacements(html_template, final, candles_json, ema5_series, ema20_series, markers, cum_pl_series, pivot_lines, trade_data)
            self._last_export_path = self.cfg.export_csv
    ...
    def _apply_template_replacements(self, html_template, final, candles_json, ema5_series, ema20_series, markers, cum_pl_series, pivot_lines, trade_data):
        ...
        html_template = (html_template
                         ...
                         .replace('__EMA5__', json.dumps(ema5_series))
                         .replace('__EMA20__', json.dumps(ema20_series))
        ...
        pivot_js = (
            ...
            "      const s = chart.addLineSeries({color:line.color,lineWidth:1,lineStyle:1,lastValueVisible:false,priceLineVisible:false,crosshairMarkerVisible:false});\n"
    ...
    def _prepare_candles(self, df):
        ...
        for ts, row in df.iterrows():
            try:
                unix_time = int(pd.Timestamp(ts).tz_localize('UTC', nonexistent='shift_forward', ambiguous='NaT').timestamp())
                candles_json.append({
                    'time': unix_time,
                    'open': float(row['open']),
                    'high': float(row['high']),
    ...
    def _prepare_pivot_lines(self, df, candles_json):
        ...
        if 'pivot_time' in df.columns:
            print("Debug: Found pivot_time column")
            pivots_full = df[['pivot_time', 'pivot_high', 'pivot_low', 'pivot_direction']].copy()
            pivots_full = pivots_full.dropna(subset=['pivot_time'])
            print(f"Debug: Non-null pivot data rows: {len(pivots_full)}")
            
            if not pivots_full.empty:
                pivots_full['pivot_time'] = pd.to_datetime(pivots_full['pivot_time'])
                grouped = []
                for ptime, grp in pivots_full.groupby('pivot_time', sort=False):
                    start_idx = grp.index[0]
                    high = grp['pivot_high'].iloc[0] if 'pivot_high' in grp else None
                    low = grp['pivot_low'].iloc[0] if 'pivot_low' in grp else None
                ...
                for i, (ptime, start_idx, high, low, direction) in enumerate(grouped):
                    block_mask = (pivots_full['pivot_time'] == ptime)
                    segment_indices = pivots_full[block_mask].index
                    if len(segment_indices) == 0:
                        continue
                    end_idx = segment_indices[-1]
                    try:
                        start_unix = int(pd.Timestamp(start_idx).tz_localize('UTC', nonexistent='shift_forward', ambiguous='NaT').timestamp())
                        end_unix = int(pd.Timestamp(end_idx).tz_localize('UTC', nonexistent='shift_forward', ambiguous='NaT').timestamp())
                    except Exception as e:
                        print(f"Debug: Failed to convert timestamps: {e}")
                        continue
                    
                    color = "#F40B0B" if direction == 'bull' else ("#06441d" if direction == 'bear' else '#ff9800')
                    if high is not None and not pd.isna(high):
                        pivot_lines.append({'start': start_unix, 'end': end_unix, 'value': float(high), 'color': "#0B1AEC"})
                        print(f"Debug: Added high pivot line at {high}")
                    if low is not None and not pd.isna(low):
                        pivot_lines.append({'start': start_unix, 'end': end_unix, 'value': float(low), 'color': "#0B1AEC"})
                        print(f"Debug: Added low pivot line at {low}")
        else:
            print("Debug: No pivot_time column found in DataFrame")
            # Check for alternative pivot data columns
            if 'pivot_high' in df.columns or 'pivot_low' in df.columns:
                print("Debug: Found pivot_high/pivot_low columns without pivot_time")
                # Use forward-filled pivot levels
                if len(candles_json) > 0:
                    for i, (ts, row) in enumerate(df.iterrows()):
                        if i < len(candles_json):
                            candle_time = candles_json[i]['time']
                            if 'pivot_high' in row and not pd.isna(row['pivot_high']):
                                pivot_lines.append({
                                    'start': candle_time,
                                    'end': candle_time + 3600,  # extend 1 hour
                                    'value': float(row['pivot_high']),
                                    'color': "#0B1AEC"
                                })
                            if 'pivot_low' in row and not pd.isna(row['pivot_low']):
                                pivot_lines.append({
                                    'start': candle_time,
                                    'end': candle_time + 3600,  # extend 1 hour
                                    'value': float(row['pivot_low']),
                                    'color': "#0B1AEC"
                                })
    ...
    def _prepare_cum_pl(self, completed_trades, df, candles_json):
        ...
        if getattr(self.cfg, 'show_pl_line', True) and completed_trades:
            ...
            closed_sorted = sorted(closed, key=lambda t: pd.to_datetime(t.get('exit_datetime')))
            running = 0.0
            pl_map = {}
            for t in closed_sorted:
                exit_dt = pd.to_datetime(t.get('exit_datetime'))
                running += float(t.get('pl', 0))
                exit_dt = exit_dt.round('5min')
            ...
            for ts in df.index:
                ...
                cum_pl_series.append({
                    'time': int(pd.Timestamp(ts).tz_localize('UTC', nonexistent='shift_forward', ambiguous='NaT').timestamp()),
                    'value': float(ff_val)
                })
