...
class AppConfig:
    ...
    def from_xml(cls, xml_path):
        ...
        # Parse all elements except strategy_params
        for elem in root:
            if elem.tag == 'strategy_params':
...
class StrategyRunner:
    ...
    def __init__(self, config: AppConfig):
        ...
        self.hdm = HistoryDataManager(db_file=f"{config.end_date}-history.db")
...
if __name__ == "__main__":
    ...
    if os.path.exists(config_xml_path):
        print(f"Loading configuration from {config_xml_path}")
        config = AppConfig.from_xml(config_xml_path)
        
        # Resolve relative paths in config
        script_dir = os.path.dirname(__file__)
        if config.db_dir and not os.path.isabs(config.db_dir):
            config.db_dir = os.path.abspath(os.path.join(script_dir, config.db_dir))
        
        # Ensure output directory exists
        if config.trades_csv:
            output_dir = os.path.dirname(config.trades_csv)
            if output_dir and not os.path.exists(output_dir):
                os.makedirs(output_dir)
        
        if config.export_csv:
            output_dir = os.path.dirname(config.export_csv)
            if output_dir and not os.path.exists(output_dir):
                os.makedirs(output_dir)
    else:
        # Fallback to hardcoded config if XML file doesn't exist
        print(f"Config file {config_xml_path} not found, using default configuration")
